...
class MusicControllerActor extends AbstractActor {
	...
	private void sendPlayCommand(MusicPlayerStartCommand startCommand) {
		if (playingSong == true) {
			System.out.println("[MusicControllerActor] : Sending switch music [" + currentPlaying + "->"
					+ startCommand.getSong() + "] action message to MusicPlayer");
		} else {
			System.out.println("[MusicControllerActor] : Sending start music action message to MusicPlayer - "
					+ startCommand.getSong());
		}
...
public class ActorCreation {
	...
	public static void main(String[] args) {
		...
		ActorRef musicController = system.actorOf(Props.create(MusicControllerActor.class, musicPlayer),
				"MusicController");
